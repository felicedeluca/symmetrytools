...
def superm2(image):
    ...
    def hex():
        ares = plt.hexbin(houghr, houghth, bins=200, cmap='inferno')

        counts=ares.get_array() #counts in each hexagon, works great
        verts=ares.get_offsets() #empty, don't use this
        b=ares.get_paths()   #this does work, gives Path([[]][]) which can be plotted
        print(b)
        for x in range(len(b)):
            xav=np.mean(b[x].vertices[0:6,0]) #center in x (RA)
            yav=np.mean(b[x].vertices[0:6,1]) #center in y (DEC)
            print(xav, yav)
        # print(sorted(ares.get_offsets()))
        # plt.show()
    hex()
    # draw(2.8, 2.4)
